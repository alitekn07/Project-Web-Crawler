...
// Database Name = MONSTER\SQLEXPRESS
...
namespace WEBCRAWLERSONPROJE
{
    public static class csHelperMethods
    {
        ...
        public class crawlingResult : tblMainUrl // varlık çerçevesi tarafından oluşturulan sınıfı devralacak ve varlık sınıfını güncellesek bile aynı kalacak
        {
            ...
            // burada değişkeni veya özelliği tanımlıyoruz tanımlamadan ayarlayamayız. Miras alınan sınıf özelliklerini veya alanlarını ayarlamak istiyorsak, yukarıdaki gibi bir kurucu(constructor) kullanılmalı.
        ...
        public static void crawlPage(string srUrlToCrawl, int irUrlDepthLevel, string _srParentUrl, DateTime _dtDiscoverDate)
        {
            ...
            saveCrawInDatabase(crawlResult);
                ...
                using (var context = new DBCrawling())
                {
                    ...
                    if (crawledResult.blcrawlSuccess == true)
                    {
                        ...
                        if (!string.IsNullOrEmpty(crawledResult.SourceCode))
                        {
                            ...
                            double dbOriginalSourceCodeLenght = crawledResult.SourceCode.Length;
                            crawledResult.SourceCode = crawledResult.SourceCode.CompressString();
                            crawledResult.CompressionPercent = Convert.ToByte(Math.Floor(((crawledResult.SourceCode.Length.ToDouble() / dbOriginalSourceCodeLenght) * 100)));
                        }
                    ...
                    if (vrResult != null)
                    {
                        ...
                        context.tblMainUrls.Add(finalObject); // burada tekrardan ekliyoruz(insert ediyoruz). Ancak .Add yerine .AddOrUpdate'de çalışıyor.
        ...
        private static string ComputeSha256Hash1(this string rawData)
        {
            // Create a SHA256   
        ...
        {
            return srUrl.normalizeUrl().ComputeSha256Hash1();
        }
        ...
        private static StreamWriter swLog = new StreamWriter("DBLogs.txt", true, Encoding.UTF8);
